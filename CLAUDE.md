...
- **Agentos** ([src/agentos/](server/src/agentos/)) — the agent layer. `POST /agents/:id/runs` ([agentos.controller.ts](server/src/agentos/agentos.controller.ts)) feeds a single `DeepAgentService`; the route's `:id` is ignored (the agent is determined by the session's bound novel). The controller walks the langgraph message stream, emits flat `ActivityEvent` frames (`think` / `content` / `tool` / `ActResult`), and closes with a `RunCompleted` frame carrying `aggregateActivities(...)` output. (The old `RunContent` cumulative frames + `WritingChapter` side-channel were removed with the swarm — do not look for them.)
  - **`DeepAgentService`** ([deep-agent.service.ts](server/src/agentos/deep-agent.service.ts)) — wraps `deepagents`' `createDeepAgent` with one main agent + three subagents (`writer` / `settler` / `validator`). The main agent delegates via `task`; subagents own the chapter/edit/memory tools. The agent is rebuilt per request (tools closure-inject `userId`/`novelId`).
  - **Model is per-user configurable** — `getModel` reads the user's active `ModelConfig` via `ModelConfigService.getActive(userId)` (one read per turn, threaded to all role instances), then `buildChatModel` ([model-factory.ts](server/src/agentos/model-factory.ts)) routes by `provider`: `openai-compatible` → `ChatOpenAI` (GLM / DeepSeek / Moonshot / Qwen / OpenAI …), `anthropic` → `ChatAnthropic`, `gemini` → `ChatGoogleGenerativeAI`. Cached by `${config.id}:${maxTokens}`; role maxTokens split (main/writer 16k, settler/validator 6k). **GLM is no longer hardcoded** — add/select a model in `/settings` (no model configured → the turn throws "尚未配置模型").
  - **Middleware** — only `excludeFilesystemTools` (filters the 7 filesystem tools `createDeepAgent` injects unconditionally: `ls`/`read_file`/`write_file`/`edit_file`/`glob`/`grep`/`execute`). The old `coerce` middleware (GLM generic-message reclassification + FS filtering, bundled) was removed when the model became configurable; switching back to GLM-5.2 may re-trigger the no-`role` chunk crash — re-add a targeted shim only if you actually hit it.
  - **Agent tools** ([src/agentos/tools/](server/src/agentos/tools/)) — main: `get_novel_info`, `update_novel`; writer: `append_section` / `replace_text` / `insert_text` / `delete_text` / `clear_chapter` / `set_chapter_title` / `get_chapter` / `list_chapters`; settler: `write_summary`; validator: `query_memory`. All LangChain `tool()` factories wrapping the mutation layer. **`userId`/`novelId` are closure-injected at build time — never read from LLM input** (security: the model cannot address another user's novel or chapter). System prompts live in [agent-prompts.ts](server/src/agentos/agent-prompts.ts).
