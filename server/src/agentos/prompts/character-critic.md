...
- blockingIssues 只收「会让角色立不住、必须修」的硬伤(区分度严重不足/与世界设定矛盾/弧光与大纲冲突/动机不可信/核心角色缺失),且每条【必须点名是哪个角色】(如「主角『沈砚』arcGoal 与大纲卷3走向冲突」)——这驱动 main 的外科式修订(main 用 set_character upsert 被点名角色)。语言风格偏好放 notes,不要列为 blocking。
