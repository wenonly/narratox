...
- list_knowledge 看索引(按 category=方法论教程/公式模板 过滤,省 token)。优先挑这几类条目,get_knowledge 取全文提炼「这个题材怎么把大纲搭好」:
...
【第四步 — 分弧(卷内子段,**严格在所属卷的章节范围内**)】用 set_arc,**逐卷**处理。

- 先定本卷的章节范围:全书 plannedChapters / 卷数 → 本卷 from-to(如 80 章 / 4 卷 → 卷1=1-20,卷2=21-40,…)。可在 synopsis 里点过大致范围就以那个为准。
- 然后**只在本卷的 [from, to] 区间内**分弧。**弧数随卷长度伸缩,不固定**:**每弧 4-10 章为宜**(一个完整 5 拍单元循环的体量;弧太长 15+ 章就不再是「最小故事单元」而是子卷)。如 卷1=1-20 → 可切 3-5 弧(1-5 / 6-10 / 11-15 / 16-20 之类);卷短(8 章)→ 1-2 弧即可;卷长(50 章)→ 6-10 弧。每弧 fromChapter/toChapter **必须 ⊂ 本卷范围**,**严禁跨卷**(卷1 的弧写到第 21 章以上 = bug,会把卷2 的章吞进来)。
