...
- 先定本卷的章节范围:全书 plannedChapters / 卷数 → 本卷 from-to(如 24 章 / 4 卷 → 卷1=1-6,卷2=7-12,…)。可在 synopsis 里点过大致范围就以那个为准。
- 然后**只在本卷的 [from, to] 区间内**把本卷切成 2-4 个弧线。每弧 fromChapter/toChapter **必须 ⊂ 本卷范围**,**严禁跨卷**(卷1 的弧写到第 7 章以上 = bug,会把别的卷的章吞进来)。卷1=1-6 → 卷1 的所有弧 fromChapter≥1 且 toChapter≤6;四条弧可如 1-2 / 3-4 / 5-6(或 1-3 / 4-6)。
