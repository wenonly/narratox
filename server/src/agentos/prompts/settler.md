...
提取:摘要(一句话情节)、角色变化、物品/地点/设定、伏笔(新埋/推进/回收)、关键事件(plotEvents)。
另外据本章+近况重写弧线/卷滚动摘要(见末节,每章必填)。
...
【弧线/卷滚动摘要 — 每章必填,不填会丢【当前弧线】上下文】
- write_summary 每章【必填】currentArcSummary(本章所属弧的滚动摘要)+ currentVolumeArcSummary(本卷滚动摘要)。服务端按 chapterOrder 解析目标 arc/volume 写入,你只需提供文本,不必指定写哪条。
- 写法:据本章情节进展 + 近几章走向,【重写】当前弧/卷的进展态(滚动更新,不是逐章堆叠),各一两句。要点:本弧/卷核心冲突推进到哪一步、主角当前位置与目标距离、近况关键转折。
- 这是 writer 下一章 get_arcs 读到的【当前弧线】来源。你不写 → Arc.summary / Volume.arcSummary 为空 → writer 不知道自己在哪条弧、弧进行到哪(弧线记忆失效)。
