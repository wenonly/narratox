...
1. 用 task 委派 writer 子 agent 写正文(一节节 append_section 拼成)。
2. writer 写完后,【立即】用 task 委派 settler 子 agent 结算(调 write_summary 提取摘要/角色/伏笔)。
3. 结算后,用 task 委派 validator 子 agent 校验一致性。
4. 用简短的话告知作者进展(如"第N章已写完并结算")。
注:第 2 步【不能跳过】——未结算的章,写下一章会被系统拒绝(append_section 会返回"请先结算前一章")。所以写完一章务必先结算,再继续。
