...
const mockPrisma = (over: {
  ...
  plannedCount?: number;
  counts?: Partial<
    Record<'ref' | 'world' | 'vol' | 'arc' | 'char' | 'event', number>
  >;
  ...
  chapter: {
    aggregate: jest.fn().mockResolvedValue({
      _max: { order: over.chapterAggMax ?? 8 },
      _count: {},
    }),
    findMany: jest
      .fn()
      .mockResolvedValue(
        over.chapters ?? [{ content: '一二三', status: 'COMMITTED', order: 1 }],
      ),
  },
  ...
  volume: {
    count: jest.fn().mockResolvedValue(over.counts?.vol ?? 1),
  ...
  chapterOutline: {
    aggregate: jest
      .fn()
      .mockResolvedValue({ _max: { chapterOrder: over.plannedMax ?? 22 } }),
  ...
  message: {
    findFirst: jest
      .fn()
      .mockResolvedValue(
        over.lastMsgActivities !== undefined
          ? { activities: over.lastMsgActivities }
          : null,
      ),
  },
});
...
const mockEvents = (
  openHooks: any[] = [{ stale: false }, { stale: true }],
) => ({
  listOpen: jest.fn().mockResolvedValue(openHooks),
});
const mockArcs = (arc: any = null) => ({
  findArcByChapter: jest.fn().mockResolvedValue(arc),
});
...
describe('StatusService.getOverview', () => {
  it('ACTIVE:进度 + frontier + 覆盖 + 健康', async () => {
    const prisma = mockPrisma({
      chapters: [
        { content: '一二三', status: 'COMMITTED', order: 1 },
        { content: '四五六七八九', status: 'COMMITTED', order: 2 },
      ],
    });
  ...
  it('currentArc 命中 + currentVolume 取 arc.volumeId', async () => {
    const prisma = mockPrisma({});
    prisma.volume.findUnique = jest
      .fn()
      .mockResolvedValue({ order: 1, title: '初入江湖' });
    const svc = new StatusService(
      prisma as any,
      mockEvents() as any,
      mockArcs({
  ...
  it('ACTIVE + plannedRemaining≤3 → nextStep plan_more', async () => {
    ...
    const prisma = mockPrisma({
      chapters: [{ content: 'x', status: 'COMMITTED', order: 20 }],
      plannedMax: 22,
    }); // remaining=22-21+1=2
    const svc = new StatusService(
      prisma as any,
  ...
  it('recentPhase 从最后一条 message activities 派生(末个工具)', async () => {
    const prisma = mockPrisma({
      lastMsgActivities: {
        steps: [{ tool: 'set_world_entry' }, { tool: 'append_section' }],
      },
    });
    const svc = new StatusService(
      prisma as any,
