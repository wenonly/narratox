...
import { SYSTEM_PROMPT } from '../src/agentos/agentos.constants';
...
// NovelService 需要一些内部依赖,直接用 prisma 模拟建书流程。
const novels = prisma; // 直接用 prisma 建测试数据
...
describe('Pipeline integration (real DB, no LLM)', () => {
  ...
  beforeAll(async () => {
    ...
    const user = await prisma.user.create({
      data: { email: TEST_EMAIL, passwordHash: 'test', username: 'pipeline-test' },
    });
    userId = user.id;

    // 建会话 + 小说 + 种子章节(模拟 NovelService.create)
    const session = await prisma.session.create({
      data: { id: `test-session-${user.id}`, userId, agentId: 'deep-agent', name: '测试小说会话' },
    });
    ...
    const novel = await prisma.novel.create({
      data: {
        ...
        genre: '玄幻',
        synopsis: '一个测试主角的冒险',
        settings: { coreConflict: '主角寻剑 vs 天命', chapterWordTarget: 1000, worldviewText: '灵气世界', style: '沉稳' },
        status: 'ACTIVE',
      },
    });
    novelId = novel.id;
  ...
  describe('foundation', () => {
    it('builds worldview entries', async () => {
      ...
      await world.upsertEntry(userId, novelId, { type: 'powerSystem', name: '灵气体系', content: '炼气→筑基→金丹→元婴' });
      await world.upsertEntry(userId, novelId, { type: 'rule', name: '天道禁忌', content: '不可逆改生死' });
    ...
    it('builds outline (volume + 2 chapter plans)', async () => {
      await outlines.upsertVolume(userId, novelId, 1, { title: '初入江湖', goal: '主角下山' });
      ...
      const { volumes, chapterOutlines } = await outlines.listOutline(userId, novelId);
    ...
    it('creates characters', async () => {
      await characters.upsertCharacter(userId, novelId, {
        name: '沈砚', role: 'PROTAGONIST', aliases: ['沈少'], background: '棺材铺少掌柜',
      });
      await characters.upsertCharacter(userId, novelId, {
        name: '陆青棠', role: 'SUPPORTING', background: '镇妖司女修士',
      });
  ...
  describe('write chapter 1 + gates', () => {
    it('appendSection passes (ch1 has plan, ch1 is first → frontier OK)', async () => {
      const res = await chapters.appendSection(userId, novelId, 1, '沈砚推开铺门，冷风灌入。');
      expect(res.ok).toBe(true);
    });

    it('appendSection ch2 blocked by assertFrontier (ch1 has content but no summary)', async () => {
      const res = await chapters.appendSection(userId, novelId, 2, '追兵到了。');
    ...
    it('appendSection ch1 without plan blocked (delete plan, test, restore)', async () => {
      // 暂时删除 ch1 的细纲
      await prisma.chapterOutline.deleteMany({ where: { novelId, chapterOrder: 1 } });
      const res = await chapters.appendSection(userId, novelId, 1, '再写一段。');
      expect(res.ok).toBe(false);
  ...
  describe('settle chapter 1', () => {
    it('writes summary + hooks (with payoffTiming) + character changes', async () => {
      ...
      await summaries.upsert({
        userId, novelId, chapterId: chapter1Id,
        summary: '沈砚获得妖刀，初显异象',
        roleChanges: [
          { name: '沈砚', field: 'personality', value: '开始警觉', reason: '目睹异象' },
      ...
      await events.createHooks(userId, novelId, [
    ...
    it('character timeline has changes + current state derived', async () => {
      ...
      expect(ch!.currentState.personality).toEqual({
        value: '开始警觉', chapterOrder: 1, reason: '目睹异象',
      });
    });
  });
  ...
  describe('after settle, ch2 gate passes', () => {
    it('appendSection ch2 passes (ch1 now settled)', async () => {
      const res = await chapters.appendSection(userId, novelId, 2, '追兵赶到巷口。');
      expect(res.ok).toBe(true);
    });
  });
  ...
  describe('context injection', () => {
    it('forSession injects 前情 + 伏笔 + 世界观', async () => {
      const { prompt, novelId: nid } = await contextAssembler.forSession(userId, sessionId);
