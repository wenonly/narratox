...
| 质检子 agent（inkos Auditor lite） | ⚠️ `validator` 存在但只评价、不触发修订 | [agent-prompts.ts:70](../server/src/agentos/agent-prompts.ts#L70) |
| 被动记忆注入（前情 + 未回收伏笔） | ✅ `ContextAssembler.forSession` | [context-assembler.service.ts:99](../server/src/agentos/context-assembler.service.ts#L99) |
