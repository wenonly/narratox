...
- **为什么是 `content` + `activities` 两字段,而不是一个自带内容的自包含文档**:为了**流式稳定**。`content` 串里只放一次性写入的静态标记 → 思考期间文档串不变,react-markdown 不重解析;动态细节(思考文本增长、工具返回填充)只更新 `activities` 查找表里那一条 → 仅对应组件重渲染、展开状态不丢、不逐 token 重解析整篇。若把细节内联进指令(单字段),流式会撞上 open 指令渲染、逐 token 重解析整篇、组件状态被冲三个问题——与"边想边显示、消除卡顿"的目标冲突。`activities` 是组件的**数据后台**,不参与 markdown 排版;渲染产物仍是一篇文档。
